...
const AllProjects: React.FC = () => {
  ...
  const categories = useMemo(
    () => [
      "all",
      ...Array.from(
        new Set(allProjectsData.map((p) => p.category).filter(Boolean))
      ),
    ],
    [allProjectsData]
  );
  const statuses = useMemo(
    () => [
      "all",
      ...Array.from(
        new Set(allProjectsData.map((p) => p.status).filter(Boolean))
      ),
    ],
    [allProjectsData]
  );
  const years = useMemo(
    () => [
      "all",
      ...Array.from(
        new Set(allProjectsData.map((p) => p.year).filter(Boolean))
      ),
    ],
    [allProjectsData]
  );
        ...
        <div className="mb-16">
          <div className="flex justify-start mb-8">
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-3 text-white rounded-lg relative group transition-colors duration-300 hover:text-gray-300"
            >
              ...
              <span className="relative inline-block">
                ...
                <span
                  className="absolute left-0 -bottom-1 h-[2px] w-full
                   bg-gradient-to-r from-purple-500 to-pink-500
                   origin-right scale-x-0 group-hover:scale-x-100 group-hover:opacity-0
                   transition-transform duration-500"
                />
              </span>
            </button>
          ...
          <div className="text-center mb-8">
            <h1 className="text-6xl md:text-8xl font-bold text-white inline-block relative group">
              ALL PROJECTS
              {/* Hover-in underline: grows left -> right */}
              <span
                className="pointer-events-none absolute left-0 -bottom-1 h-[3px] w-full
                 bg-gradient-to-r from-purple-500 to-pink-500
                 origin-left scale-x-0 group-hover:scale-x-100
                 transition-transform duration-500
                 opacity-0 group-hover:opacity-100"
              />
              {/* Hover-out underline: slides off to the right */}
              <span
                className="pointer-events-none absolute left-0 -bottom-1 h-[3px] w-full
                 bg-gradient-to-r from-purple-500 to-pink-500
                 origin-right scale-x-0 group-hover:scale-x-100 group-hover:opacity-0
